...
// ************************ nastavení ************************
#define SHOW_FULL_TEXT 1    
#define SHOW_TEXT_ONCE 1    
...
void setTimersForMode(byte aMode) {
  if (runningFlag || aMode == DEMO_TEXT_0 || aMode == DEMO_TEXT_1 || aMode == DEMO_TEXT_2) {
    ...
    // Это эффект Пейнтбол
    effectSpeed = getEffectSpeed(EFFECT_PAINTBALL);  
...
void checkIdleState() {
  ...
  if (idleState) {
    ...
    if ((millis() - autoplayTimer > autoplayTime) && AUTOPLAY) {    
      autoplayTimer = millis();
      if (modeCode == MC_TEXT && SHOW_FULL_TEXT) {   
        if (fullTextFlag) {
          fullTextFlag = false;
          nextMode();
        }
...
byte mapEffectToAlarm(byte effect) {
  ...
  return sizeof(ALARM_LIST_IDX) - 1; 
}
